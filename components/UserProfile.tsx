      ...
      <div className="bg-white rounded-2xl shadow-sm border p-8 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
         ...
         <div className="relative group">
            <div className="w-32 h-32 rounded-full border-4 border-white shadow-xl overflow-hidden bg-slate-100 flex items-center justify-center">
                {currentUser.avatar ? (
                    ...
                    <span className="text-4xl font-bold text-slate-300">{currentUser.name.charAt(0)}</span>
                )}
            </div>
         ...
         <div className="text-center md:text-start z-10">
             ...
             <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-2">
                 <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm font-semibold">{currentUser.role}</span>
                 {company && (
                     <span className={`px-3 py-1 bg-${company.color}-50 text-${company.color}-700 rounded-full text-sm font-semibold flex items-center gap-1`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-4">
                  <div>
                      <label className="text-xs text-gray-500 uppercase font-bold">{t('username')}</label>
                      <div className="p-3 bg-gray-50 rounded border mt-1 font-mono text-gray-700">{currentUser.username}</div>
                  </div>
                  <div>
                      <label className="text-xs text-gray-500 uppercase font-bold">{t('email')}</label>
                      <div className="p-3 bg-gray-50 rounded border mt-1 text-gray-700 flex items-center gap-2">
                          <Mail className="w-4 h-4 text-gray-400"/> {employeeData?.email || '-'}
                      </div>
                  </div>
                  <div>
                      <label className="text-xs text-gray-500 uppercase font-bold">{t('phone_number')}</label>
                      <div className="p-3 bg-gray-50 rounded border mt-1 text-gray-700">{employeeData?.phone || '-'}</div>
                  </div>
                  <div>
                      <label className="text-xs text-gray-500 uppercase font-bold">{t('assigned_company')}</label>
                      <div className="p-3 bg-gray-50 rounded border mt-1 text-gray-700">{company?.fullName || '-'}</div>
                  </div>
              </div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border p-6">
                  ...
                  {passMessage && (
                      <div className={`p-3 rounded mb-4 text-sm flex items-center gap-2 ${passMessage.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                  ...
                  <div className="space-y-3">
                      ...
                      <button 
                        ...
                        className="w-full bg-slate-800 text-white py-2 rounded font-bold hover:bg-slate-700 disabled:opacity-50 transition"
                      >
                          {t('save')}
                      </button>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border p-6">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                       {employeeData?.notifications?.map(note => (
                           <div key={note.id} className="bg-yellow-50 p-3 rounded border border-yellow-100 text-sm">
                               <p className="text-slate-800 font-medium">{note.message}</p>
                               <span className="text-xs text-slate-500 mt-1 block">{note.date}</span>
                           </div>
                       ))}
                       {(!employeeData?.notifications || employeeData.notifications.length === 0) && (
                           <p className="text-gray-400 text-center py-4 text-sm">{t('no_notifications')}</p>
                       )}
                  </div>
              </div>
          </div>
